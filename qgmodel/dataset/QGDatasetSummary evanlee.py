...
class QGDatasetSummary(torch.utils.data.Dataset):
    def __init__(self, dataset_name, tokenizer_name, input_max_len, model_type, train=True, token=None, ignore_index=-100):
        ...
        self.summtokenizer = PreTrainedTokenizerFast.from_pretrained("EbanLee/kobart-summary")
        self.summmodel = BartForConditionalGeneration.from_pretrained("EbanLee/kobart-summary")
    ...
    def __getitem__(self, idx):
        ...
        if self.model_type == 'BART':
            input_ids = self.summtokenizer.encode(instance['context'], return_tensors="pt", truncation=True)
            summary_text_ids = self.summmodel.generate(
                                    input_ids=input_ids,
                                    bos_token_id=self.summmodel.config.bos_token_id,
                                    eos_token_id=self.summmodel.config.eos_token_id,
                                    length_penalty=2.0,
                                    max_length=256,
                                    min_length=10,
                                    num_beams=6,
            ...
            inscont = self.summtokenizer.decode(summary_text_ids[0], skip_special_tokens=True)
            tokenized_input = self.tokenizer(inscont + self.tokenizer.sep_token + instance['answer'], 
